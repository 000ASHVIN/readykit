      ...
      <div class="col-12 mb-primary">
        <div class="card card-with-shadow border-0 h-100">
          <div class="card-header p-primary bg-transparent">
            <h5 class="card-title m-0">Create Area</h5>
          </div>
          <div class="card-body">
            <form class="mb-0">
              ...
              <div class="mt-5 action-buttons">
                <a @click="createArea" class="btn btn-primary mr-2">
                  Add Area
                </a>
              </div>
            </form>
...
export default {
  ...
  methods: {
      ...
      async createArea(){
        ...
        .then((response) => {
          if(response.data){
              this.$alert("Area created !!");
              window.location.replace('/admin/areas');
          }
          else{
              this.$alert("There's Problem creating Area !!");
          }
        })
        .catch(({ response }) => {
